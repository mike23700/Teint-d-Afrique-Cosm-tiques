...
// NB : deux placeholders distincts — avec ATTR_EMULATE_PREPARES => false (bootstrap.php),
// un même placeholder nommé ne peut pas apparaître deux fois dans une requête.
$refStmt = $pdo->prepare(
    'SELECT (SELECT COUNT(*) FROM gammes WHERE image_id = :id_gamme)
          + (SELECT COUNT(*) FROM produits WHERE image_id = :id_produit) AS refs'
);
$refStmt->execute(['id_gamme' => $id, 'id_produit' => $id]);
