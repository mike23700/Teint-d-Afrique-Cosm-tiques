...
const IMAGE_LABELS: Record<string, string> = {
  ...
  'accueil.fondatrice_image': 'Portrait de la fondatrice',
}
  ...
  if (block.blockType === 'image') {
    return (
      <div className="bg-white border border-[#3B1705]/10 p-5 space-y-3">
        <ImagePicker
          label={IMAGE_LABELS[`${page}.${block.blockKey}`] ?? block.blockKey}
          currentImage={imageUrl}
          fallbackImage={DEFAULT_CONTENT_IMAGES[`${page}.${block.blockKey}`]}
          onPick={async media => {
            setSuccess(false)
            await updateContent(page, block.blockKey, media ? String(media.id) : '')
            setValue(media ? String(media.id) : '')
